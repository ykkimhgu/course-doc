...
// State table definition
typedef struct {
  uint32_t out[2][2][2];     // output = FSM[state].out[input X][input Y][PWM or LED]
...
void nextState(){
  ...
  pwmOut = FSM[state].out[input[0]][input[1]][PWM];
  ledOut = FSM[state].out[input[0]][input[1]][LED];
  
  nextstate = FSM[state].next[input[0]][input[1]];
